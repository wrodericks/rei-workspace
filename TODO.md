...
- [ ] **MCP experimentation** — build or connect an MCP server personally to understand the standard hands-on before deploying at work. Good candidate: a simple actuarial lookup tool or the IFRS 17 agent as an MCP server. Also write one from scratch — best way to understand the full request/response cycle.
...
- [ ] **GitHub setup (Aug 15 morning)** — create GitHub account, set up git on DeepThought, create first repo (ifrs17-rag-agent). Learn the basics: clone, commit, push, README. Foundation for sharing all future projects.
- [ ] **Build first RAG / domain agent** — hands-on practice before Sep 9. Recommended first project: IFRS 17 RAG agent (public doc, known domain, directly relevant). Stack: Python + LangChain or LlamaIndex + Chroma (local vector DB) + Claude/OpenAI API. No GPU or fine-tuning needed — chunk, embed, retrieve, generate. One afternoon to first working prototype.
- [ ] **Process intake template** — design standard interview template/question set for collecting consistent process information. Sources: BPMN 2.0 spec (what info is needed to model), BABOK (elicitation best practices), Lean/Six Sigma (value stream + DMAIC questions), CIA Standards (actuarial controls). Sections: process overview, inputs/outputs, systems, decision points, volumes, pain points, controls/compliance, handoffs.
- [ ] **Establish risk-adjusted profitability metric** — assess current state, align with CFO/finance on RAROC + VNB/CSM framework, embed in pricing governance agent. See details/risk-adjusted-profitability.md
...
- [ ] **Contact Evident Insights** — reach out re: Evident AI Index for Insurance. Discuss how to get company on the list and improve ranking. They track serious adopters proactively.
